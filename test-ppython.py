...
# Function to play audio for navigation instruction
def play_instruction(instruction):
    audio_path = f'instructions/{instruction}.mp3'
    if os.path.exists(audio_path):
        print(f"Playing instruction: {instruction}")
        pygame.mixer.music.load(audio_path)
        pygame.mixer.music.play()
        while pygame.mixer.music.get_busy():  # Wait until the audio finishes playing
            time.sleep(0.1)
    else:
        print(f"Audio file for {instruction} not found.")
...
def determine_direction(x, width):
    if x < width // 3:
        return "turn left"
    elif x > 2 * width // 3:
        return "turn right"
        ...
        if signal == "DETECT":
            while True:
                ...
                for box, class_id, confidence in detections:
                    ...
                    direction = determine_direction(x, frame.shape[1])
                    play_instruction(direction)  # Play instruction for the direction
